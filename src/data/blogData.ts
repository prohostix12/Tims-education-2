...
function resolveItemDates(item: any) {
  ...
  return {
    ...
    month: parsed.month,
    year: parsed.year,
    dateString: item.dateString && typeof item.dateString === "string" && item.dateString.trim() ? item.dateString.trim() : parsed.dateString,
  };
}
...
export async function fetchFeaturedHomeBlogPostsFromDb(limit = 3): Promise<BlogPost[]> {
  try {
    ...
    // If no posts are specifically marked, fall back to recent published posts in DB
