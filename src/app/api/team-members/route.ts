...
export async function POST(request: Request) {
  let body: CreateTeamMemberPayload;
  try {
  ...
  if (typeof name !== "string" || name.trim().length === 0) {
    return NextResponse.json({ error: "Team member full name is required." }, { status: 400 });
  }

  if (typeof role !== "string" || role.trim().length === 0) {
    return NextResponse.json({ error: "Team member designation / role is required." }, { status: 400 });
  }
  ...
  const doc = {
    ...
    accentBg: typeof accentBg === "string" && accentBg.trim().length > 0 ? accentBg.trim() : "#2563eb",
  ...
  try {
    ...
    return NextResponse.json({ error: "Could not save team member profile." }, { status: 500 });
  }
}
