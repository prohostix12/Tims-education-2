...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: "Could not load team member profile." }, { status: 500 });
  }
}
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, message: "Team member profile updated successfully." });
  } catch (error) {
    console.error("Failed to update team member:", error);
    return NextResponse.json({ error: "Could not update team member profile." }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, message: "Team member profile deleted successfully." });
  } catch (error) {
    console.error("Failed to delete team member:", error);
    return NextResponse.json({ error: "Could not delete team member profile." }, { status: 500 });
  }
}
