...
// Admin Diploma Programs Management
...
function DiplomaAdminPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/diploma-streams")
      ...
      .catch((err) => console.error("Failed to load diploma streams:", err));
  }, []);
  ...
  useEffect(() => {
    if (streams.length > 0) {
      try {
        localStorage.setItem("tims_diploma_streams", JSON.stringify(streams));
      } catch {}
    }
  }, [streams]);
  ...
  const handleSaveStream = async () => {
    ...
    if (editingStreamId) {
      ...
      try {
        const res = await fetch(`/api/diploma-streams/${editingStreamId}`, {
          method: "PUT",
        ...
        showToast(`Updated Category "${formStreamLabel.trim()}"`);
      } catch (err) {
        console.error("Failed to update stream:", err);
      }
    } else {
      try {
        const res = await fetch("/api/diploma-streams", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
        ...
        showToast(`Created Category "${formStreamLabel.trim()}"`);
      } catch (err) {
  ...
  const handleOpenAddCourseModal = () => {
    if (!activeStream) {
      alert("Please create or select a diploma category first.");
      return;
    }
  ...
  const handleSaveCourse = async () => {
    if (!formCourseName.trim()) {
      alert("Please enter a diploma course name.");
      return;
    }
    ...
    if (editingCourseId) {
      ...
      try {
        const res = await fetch(`/api/diploma-streams/${sId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  const handleMoveCourseOrder = async (courseId: string, direction: "up" | "down") => {
    ...
    try {
      await fetch(`/api/diploma-streams/${sId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deleteCandidate.type === "stream") {
      ...
      try {
        ...
        showToast(`Deleted Category "${deleteCandidate.name}"`);
      } catch (err) {
      ...
      try {
        await fetch(`/api/diploma-streams/${sId}?courseId=${cId}`, {
          method: "DELETE",
        });
  ...
  return (
    <div className={styles.container}>
      ...
      <nav className={styles.breadcrumb}>
        ...
        <span className={styles.breadcrumbSeparator}>/</span>
        <span className={styles.breadcrumbActive}>Diploma Programs</span>
      </nav>

      {/* Header Bar */}
      <div className={styles.headerBar}>
        <div className={styles.headerMain}>
          <h1 className={styles.heading}>Diploma Programs</h1>
          <p className={styles.subtitle}>
            Manage Diploma categories, streams, and course eligibility tables shown in the user panel.
          </p>
        </div>
      ...
      <div className={styles.statsGrid}>
        ...
        <div className={styles.statCard}>
          ...
          <div className={styles.statInfo}>
            ...
            <span className={styles.statLabel}>Total Diploma Courses</span>
          </div>
        </div>
      ...
      <div className={styles.streamsSection}>
        <div className={styles.streamTabsHeader}>
          <h3 className={styles.streamTabsTitle}>Select Diploma Category</h3>
          <span className={styles.streamCountBadge}>{streams.length} Categories</span>
        </div>
      ...
      <div className={styles.tableSection}>
        ...
        <div className={styles.controlsBar}>
          <div className={styles.activeStreamHeaderInfo}>
            <h2 className={styles.activeStreamTitle}>{activeStream?.label || "No Category Selected"}</h2>
            <span
                ...
                <tr>
                  <td colSpan={5} className={styles.emptyCell}>
                    <div className={styles.emptyState}>
                      ...
                      <p>No diploma courses found for this category.</p>
                      <button
                        onClick={handleOpenAddCourseModal}
                        className={styles.secondaryBtn}
                        style={{ marginTop: "0.5rem" }}
                      >
                        Add First Diploma Course
                      </button>
                    </div>
                  </td>
                </tr>
          ...
          <div className={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>
                {editingStreamId ? "Edit Category" : "Add New Category"}
              </h3>
            ...
            <div className={styles.modalBody}>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>Category Title *</label>
                <input
                  type="text"
                  placeholder="e.g. Diploma or Diploma in Engineering"
            ...
            <div className={styles.modalFooter}>
              ...
              <button className={styles.primaryBtn} onClick={handleSaveStream}>
                {editingStreamId ? "Save Changes" : "Create Category"}
              </button>
            </div>
          </div>
        </div>
      ...
      {isCourseModalOpen && (
        <div className={styles.modalOverlay} onClick={() => setIsCourseModalOpen(false)}>
          ...
          >
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>
                {editingCourseId ? "Edit Diploma Course" : "Add New Diploma Course"}
              </h3>
            ...
            <div className={styles.drawerGrid}>
              ...
              <div className={styles.modalBody}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Mechanical or Civil"
                ...
                <div className={styles.formGroup}>
                  ...
                  <textarea
                    rows={3}
                    placeholder="e.g. 10th Pass or 10+2 or Its Equivalent"
              ...
              <div className={styles.previewContainer}>
                <div className={styles.previewHeader}>
                  ...
                  <span className={styles.previewSub}>
                    Exact rendering in Diploma course list table
                  </span>
                </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setDeleteCandidate(null)}>
          <div className={styles.confirmModal} onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className={styles.confirmTitle}>
              Delete {deleteCandidate.type === "stream" ? "Category" : "Course"}?
            </h3>
            <p className={styles.confirmMessage}>
              Are you sure you want to delete <strong>"{deleteCandidate.name}"</strong>?{" "}
              {deleteCandidate.type === "stream" &&
                "Deleting a category will also remove all associated courses in it."}
            </p>
...
export default function DiplomaAdminPageWrapper() {
  return (
    <Suspense fallback={<div style={{ padding: "2rem", color: "#64748b" }}>Loading Diploma Programs Management...</div>}>
      <DiplomaAdminPage />
    </Suspense>
  );
}
